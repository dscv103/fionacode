...
The GitHub MCP server works with a Personal Access Token (no OAuth). Set an auth header such as `"Authorization": "Bearer {env:GITHUB_MCP_PAT}"` and make sure `GITHUB_MCP_PAT` is present in your shell/CI environment—if it is missing, the header may be empty and the server will reject requests without a clear prompt.
