...
export default tool({
  ...
  async execute(args, context) {
    ...
    if (args.action === "set_status") {
      ...
      touch()
      await saveState(filePath, state)
      return { ok: true, task_id: task.id, status: task.status, path: filePath }
    }
    ...
    if (args.action === "clear_blocker") {
      const task = requireTask(state, args.task_id)
      if (!args.blocker_id) throw new Error("clear_blocker requires blocker_id")
      ...
      return { ok: true, task_id: task.id, blockers: task.blockers, path: filePath }
    }
