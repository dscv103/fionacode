...
async function runCommand(command: string[], timeoutMs = 180_000): Promise<RunResult> {
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      clearTimeout(timer)
      resolve({
        checker: command[0] === "pyright" || command[0] === "python3" ? "pyright" : "ty",
        command,
...
function dedupeDiagnostics(diags: Diagnostic[]): Diagnostic[] {
  ...
  for (const d of diags) {
    const key = [d.checker, d.path, d.line, d.column, d.code ?? "", d.message].join("|")
...
function groupByFile(diags: Diagnostic[]) {
  ...
  return Array.from(files.entries())
    ...
    .sort((a, b) => b.counts.error - a.counts.error || b.counts.warning - a.counts.warning || a.path.localeCompare(b.path))
}

function parsePyrightJSON(stdout: string): { diagnostics: Diagnostic[]; parseError?: string } {
  try {
    ...
    return { diagnostics: [], parseError: err?.message ?? "Failed to parse pyright JSON" }
  }
}

const TY_TEXT_RE = /^(?<path>[^:]+):(\d+):(\d+):\s*(?<severity>error|warning|info)?\s*(?<code>[A-Za-z0-9_\-\.]+)?\s*-?\s*(?<message>.+)$/i

function parseTy(stdout: string, stderr: string): { diagnostics: Diagnostic[]; parseError?: string } {
  const diagnostics: Diagnostic[] = []
  ...
  return { diagnostics: dedupeDiagnostics(diagnostics), parseError: lines.length && diagnostics.length === 0 ? "Unrecognized ty output format" : undefined }
}
...
export default tool({
  ...
  args: {
    profile: tool.schema.enum(PROFILES).describe("Which checker profile to run: pyright, ty, or both"),
  ...
  async execute(args) {
    ...
    const anyCommandFailed = runs.some((r) => r.exitCode !== 0 || r.exitCode === null || r.exitCode === -1)
    const ok = true
