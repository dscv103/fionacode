...
async function getStagedFiles(timeoutMs: number): Promise<string[]> {
  const result = await runCommand(["git", "diff", "--cached", "--name-only"], timeoutMs)
...
function analyzeChanges(
  ...
  const hasDocs = files.some((f) => f.includes("README") || f.includes("docs/") || f.endsWith(".md"))
  const hasConfig = files.some((f) => f.includes("config") || f.endsWith(".json") || f.endsWith(".yaml") || f.endsWith(".toml"))
  const hasCI = files.some((f) => f.includes(".github") || f.includes("ci/") || f.includes(".gitlab"))
  const hasSrc = files.some((f) => f.endsWith(".py") || f.endsWith(".js") || f.endsWith(".ts"))

  // Analyze diff content
  const hasNewFeature = diff.includes("+def ") || diff.includes("+class ") || diff.includes("+async def")
...
export default tool({
  ...
  args: {
    custom_subject: tool.schema
      .string()
      .optional()
      .describe("Optional custom subject line to use instead of auto-generated"),

    custom_scope: tool.schema
      .string()
      .optional()
      .describe("Optional scope to include in commit message (e.g., 'api', 'auth', 'ui')"),
